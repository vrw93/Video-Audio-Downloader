...
class jenisFileFrame(ctk.CTkFrame):
    def __init__(self, frame):
        super().__init__(frame)

        # Jenis file
        jenisTitle = ctk.CTkLabel(self, font=("Arial", 18), text="Pilih Jenis File")
        jenisTitle.grid(row=3, column=0, padx=20, sticky="w")

        jenis = ["Video", "Audio"]
        self.jenisTerpilih = ctk.StringVar(value=jenis[0])
        self.jenisOption = ctk.CTkOptionMenu(self, values=jenis, variable=self.jenisTerpilih)
        self.jenisOption.grid(row=4, column=0, padx=20, pady=25, sticky="w")
...
class mainGUI(ctk.CTk):
    def __init__(self):
        ...
        # Jenis file frame
        self.jenisFrame = jenisFileFrame(self)
        self.jenisFrame.grid(row=3, column=0, padx=20, sticky="ew")
        ...
        def download(formatd):
            try:
                ...
                if self.jenisFrame.jenisTerpilih.get() == "Video":
                ...
                self.jenisFrame.formatEntry.insert(0, "")
                self.after(1200, lambda: self.downloadLabel.configure(text="No Active Download"))

        def asyncdownload():
            if self.jenisFrame.formatEntry.get() == "" and self.jenisFrame.jenisOption.get() == "Video":
                self.jenisFrame.formatEntry.insert(0, "mp4")
            elif self.jenisFrame.formatEntry.get() == "" and self.jenisFrame.jenisOption.get() == "Audio":
                self.jenisFrame.formatEntry.insert(0, "mp3")
            ...
            if self.urlentry.get() != "":
                ...
                thread = threading.Thread(target=download, args=(self.jenisFrame.formatEntry.get(),), daemon=True)
                thread.start()
